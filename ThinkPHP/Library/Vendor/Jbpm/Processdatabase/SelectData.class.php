...
class SelectData{
    ...
    public function getDataFromDataBaseByExecution($varExecution){
        $data = M('think_workflow_execution')->alias('e')
            ...
            ")
            ->join('left join think_workflow_histactinst AS ha ON e.hisactinst = ha.dbid')
            ->join('left join think_workflow_histprocinst AS hp ON e.instance = hp.dbid')
            ->join('left join think_workflow_modulerule AS mr ON e.procdefid = mr.rulename')
            ->join('left join think_workflow_task AS t ON e.dbid = t.execution')
            ->join('left join think_workflow_histtask AS ht ON t.dbid = ht.dbid')
